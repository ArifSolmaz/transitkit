...
class LightCurveData:
    ...
    def label(self) -> str:
        ...
        parts = [self.mission]
        if self.sector: parts.append(f"S{self.sector}")
        if self.quarter: parts.append(f"Q{self.quarter}")
        if self.campaign: parts.append(f"C{self.campaign}")
        if self.cadence: parts.append(self.cadence)
        if self.author: parts.append(self.author)
        return "_".join(parts)
...
class MultiMissionDownloader:
    ...
    def __init__(self, target, cache_dir: Optional[str] = None):
        ...
        self._data = MultiMissionData(
            target_name=target.ids.planet_name or target.identifier,
            tic=target.tic,
            kic=target.kic
        )
    ...
    def _download_tess(self):
        ...
        for i, row in enumerate(search_result):
            try:
                ...
                author = getattr(lc.meta, 'AUTHOR', str(row.author) if hasattr(row, 'author') else None)
                ...
                lc_data = LightCurveData(
                    ...
                    flux_err=np.array(lc.flux_err.value) if lc.flux_err is not None else np.zeros_like(lc.flux.value),
                    mission='TESS',
                    sector=sector,
                    cadence=cadence,
                    author=author,
    ...
    def _download_kepler(self):
        ...
        if not kic:
            ...
            if self.target.stellar.ra and self.target.stellar.dec:
                search_result = lk.search_lightcurve(
                    f"{self.target.stellar.ra} {self.target.stellar.dec}",
                    mission='Kepler'
                )
            else:
        ...
        for i, row in enumerate(search_result):
            try:
                ...
                lc_data = LightCurveData(
                    ...
                    mission='Kepler',
                    quarter=quarter,
                    cadence='long' if getattr(lc.meta, 'OBSMODE', '') == 'long cadence' else 'short',
                    metadata=dict(lc.meta) if lc.meta else {}
                )
    ...
    def _download_jwst(self):
        ...
        try:
            ...
            jwst_mask = (
                ...
                (obs_table['dataproduct_type'].astype(str).str.contains('spectrum', case=False, na=False))
            )
                ...
                try:
                    ...
                    if len(x1d_products) > 0:
                        ...
                        manifest = Observations.download_products(
                            x1d_products,
                            download_dir=str(self.cache_dir / 'jwst')
                        )
                    ...
                    if ext.name == 'EXTRACT1D' or (hasattr(ext, 'data') and ext.data is not None):
                        if hasattr(ext.data, 'dtype') and 'WAVELENGTH' in ext.data.dtype.names:
                            spectrum = SpectrumData(
                                ...
                                flux=ext.data['FLUX'],
                                flux_err=ext.data['FLUX_ERROR'] if 'FLUX_ERROR' in ext.data.dtype.names else np.zeros_like(ext.data['FLUX']),
    ...
    def download_tess(self, sectors: Optional[List[int]] = None, cadence: Optional[str] = None) -> List[LightCurveData]:
        """
