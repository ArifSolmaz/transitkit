...
class UniversalResolver:
    ...
    def _init_simbad(self):
        ...
        if self._simbad is None:
            ...
            self._simbad.add_votable_fields(
                'ids', 'flux(V)', 'flux(K)', 'sp', 'plx', 'distance',
                'fe_h', 'otype'
            )
        return self._simbad
    ...
    def _resolve_from_koi(self, koi_id: str, result: Dict):
        ...
        try:
            koi_data = NasaExoplanetArchive.query_criteria(
                table="koi",
                where=f"kepoi_name LIKE 'K{koi_id}%'"
            )
    ...
    def _resolve_from_name(self, name: str, result: Dict):
        ...
        if result['stellar'].ra and result['stellar'].dec:
            try:
                coord = SkyCoord(
                    ra=result['stellar'].ra, 
                    dec=result['stellar'].dec, 
                    unit='deg'
                )
    ...
    def _enrich_from_simbad(self, result: Dict, name: Optional[str] = None):
        ...
        try:
            ...
            if 'SP_TYPE' in row.colnames:
                result['stellar'].spectral_type = (
                    result['stellar'].spectral_type or 
                    str(row['SP_TYPE'])
                )
...
class UniversalTarget:
    ...
    def _print_resolution_summary(self):
        ...
        # IDs
        print("\n📋 Identifiers:")
        if ids.tic: print(f"   TIC: {ids.tic}")
        if ids.kic: print(f"   KIC: {ids.kic}")
        if ids.toi: print(f"   TOI: {ids.toi}")
        if ids.koi: print(f"   KOI: {ids.koi}")
        if ids.hd: print(f"   HD: {ids.hd}")
        if ids.gaia_dr3: print(f"   Gaia DR3: {ids.gaia_dr3}")
        
        # Stellar params
        print("\n⭐ Host Star:")
        if stellar.teff: print(f"   Teff: {stellar.teff:.0f} K")
        if stellar.radius: print(f"   Radius: {stellar.radius:.3f} R☉")
        if stellar.mass: print(f"   Mass: {stellar.mass:.3f} M☉")
        if stellar.distance: print(f"   Distance: {stellar.distance:.1f} pc")
        
        # Planets
        if r['planets']:
            print(f"\n🪐 Planets ({len(r['planets'])} found):")
            for p in r['planets']:
                print(f"   {p.name}: P={p.period:.4f}d, Rp={p.radius:.2f} R⊕" 
                      if p.period and p.radius else f"   {p.name}")
