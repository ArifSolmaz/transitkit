...
class PublicationGenerator:
    ...
    def generate_all(self, output_dir: str):
        ...
        if self.results.get('lightcurves'):
            self.generate_lightcurve_figure(
                output_path / 'figures' / 'lightcurve.pdf'
            )
        
        if self.results.get('candidates'):
            self.generate_transit_figure(
                output_path / 'figures' / 'transit.pdf'
            )
        
        if self.results.get('transmission_spectrum'):
            self.generate_spectrum_figure(
                output_path / 'figures' / 'transmission.pdf'
            )
    ...
    def _aastex_stellar_table(self, stellar, ids) -> str:
        ...
        if stellar.teff:
            ...
            lines.append(rf"$T_{{\rm eff}}$ (K) & {stellar.teff:.0f}{err} & {stellar.source or 'TIC'} \\")
        
        if stellar.logg:
            err = f" $\\pm$ {stellar.logg_err:.2f}" if stellar.logg_err else ""
            lines.append(rf"$\log g$ (cgs) & {stellar.logg:.2f}{err} & {stellar.source or 'TIC'} \\")
        ...
        if stellar.radius:
            ...
            lines.append(rf"$R_\star$ ($R_\odot$) & {stellar.radius:.3f}{err} & {stellar.source or 'TIC'} \\")
        
        if stellar.mass:
            err = f" $\\pm$ {stellar.mass_err:.3f}" if stellar.mass_err else ""
            lines.append(rf"$M_\star$ ($M_\odot$) & {stellar.mass:.3f}{err} & {stellar.source or 'TIC'} \\")
    ...
    def _generic_stellar_table(self, stellar, ids) -> str:
        ...
        if ids.tic:
            lines.append(rf"TIC ID & {ids.tic} & TIC \\")
        if stellar.teff:
            lines.append(rf"$T_{{\rm eff}}$ (K) & {stellar.teff:.0f} & {stellar.source or 'TIC'} \\")
        if stellar.radius:
            lines.append(rf"$R_\star$ ($R_\odot$) & {stellar.radius:.3f} & {stellar.source or 'TIC'} \\")
        if stellar.mass:
            lines.append(rf"$M_\star$ ($M_\odot$) & {stellar.mass:.3f} & {stellar.source or 'TIC'} \\")
    ...
    def generate_transit_figure(self, filepath: str):
        ...
        ax.plot(phase[transit_mask] * candidate.period * 24, 
                combined.flux[transit_mask], 'k.', markersize=2, alpha=0.5)
        ax.set_xlabel('Time from mid-transit (hours)', fontsize=12)
        ax.set_ylabel('Normalized Flux', fontsize=12)
        ax.set_title(f'Transit (P={candidate.period:.4f} d, depth={candidate.depth:.0f} ppm)', fontsize=12)
    ...
    def generate_spectrum_figure(self, filepath: str):
        ...
        ax.errorbar(
            spectrum.wavelength,
            spectrum.depth_ppm,
            yerr=spectrum.transit_depth_err * 1e6 if np.median(spectrum.transit_depth_err) < 0.1 else spectrum.transit_depth_err,
            fmt='o',
            markersize=4,
            capsize=2,
            color='navy',
